...
public class CircularLayout extends ViewGroup {
    ...
    private void parseAttrs(Context context, AttributeSet attrs) {
        int defaultSpacing = (int) (context.getResources().getDisplayMetrics().density * 5 + 0.5f);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        float offsetAngle = averageAngle > 0 && childCount % 2 == 0 ? averageAngle : 0;    // 偏移角度，有助于让整个图形左右对称
        for (int position = 0, size = getChildCount(); position < size; position++) {
            ...
            switch (calculateLocationByAngle(nextAngle)){
                case LOCATION_NORTH ://东 左
                    childViewLeft -= childViewMeasuredWidth / 2;
                    childViewTop -= childViewMeasuredHeight;
                    childViewTop -= spacing;
                    break;
                case LOCATION_EAST_NORTH :// 右上角
                    childViewTop -= childViewMeasuredHeight / 2;

                    childViewLeft += spacing;
                    childViewTop -= spacing;
                    break;
                case LOCATION_EAST ://西 右
                    childViewTop -= childViewMeasuredHeight / 2;

//                     childViewLeft += spacing;
                    break;
                case LOCATION_EAST_SOUTH :// 右下角
                    childViewLeft += spacing;
                   childViewTop -= spacing;// childViewTop += spacing;//
                    break;
                case LOCATION_SOUTH :// 北 上
                    childViewLeft -= childViewMeasuredWidth / 2;

                    childViewTop += spacing;
                    break;
                case LOCATION_WEST_SOUTH ://左下角
                    childViewLeft -= childViewMeasuredWidth;

                    childViewLeft -= spacing;
                   childViewTop -= spacing;//  childViewTop += spacing;//
                    break;
                case LOCATION_WEST ://南 下 左边
                    childViewLeft -= childViewMeasuredWidth;
                    childViewTop -= childViewMeasuredHeight / 2;

//                    childViewLeft -= spacing;
                    break;
                case LOCATION_WEST_NORTH ://左上角
                    childViewLeft -= childViewMeasuredWidth;
                    childViewTop -= childViewMeasuredHeight / 2;
