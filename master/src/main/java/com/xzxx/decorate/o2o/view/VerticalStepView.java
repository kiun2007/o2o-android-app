...
public class VerticalStepView extends LinearLayout implements VerticalStepViewIndicator.OnDrawIndicatorListener {
    ...
    private int mUnComplectedTextColor = ContextCompat.getColor(getContext(), R.color.font_gray);
    ...
    private void init() {
        View rootView = LayoutInflater.from(getContext()).inflate(R.layout.master_widget_vertical_stepsview, this);
    ...
    public VerticalStepView setStepViewTimeTexts(List<String> texts) {
        mTextTime = texts;
        return this;
    }
    ...
    public VerticalStepView setDate(String date) {
        this.date = date;
        return this;
    }
        ...
        if (mTextContainer != null) {
            ...
            if (mTexts != null && complectedXPosition != null && complectedXPosition.size() > 0) {
                for (int i = 0; i < mTexts.size(); i++) {
                    ...
                    mTimeTextView.setTextSize(TypedValue.COMPLEX_UNIT_SP, mTextSize - 2);
                    mTimeTextView.setText("预计" + date + "前到账");
                    mTextView.setTextColor(mUnComplectedTextColor);
                    mTimeTextView.setLayoutParams(new ViewGroup.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT));
                    if (i == mComplectingPosition) {
                        ...
                        linearLayout.setY((complectedXPosition.get(i) - mStepsViewIndicator.getCircleRadius() / 2) - 17);
                    }

                    mTextContainer.addView(linearLayout);
                }
            }
        }
