...
public class VerticalStepViewIndicator extends View {
    ...
    //  definition mUnCompletedLineColor
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        for (int i = 0; i < mStepNum; i++) {
            ...
            if (mIsReverseDraw) {
                mCircleCenterPointPositionList.add(mHeight - (mCircleRadius + i * mCircleRadius * 2 + i *
                        mLinePadding));
            } else {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mCircleCenterPointPositionList.size() - 1; i++) {
            ...
            if (i < mComplectingPosition)//判断在完成之前的所有点
            {
                ...
                if (mIsReverseDraw) {
                    canvas.drawRect(mCenterX, afterComplectedXPosition + mCircleRadius - 10, mCenterX,
                            preComplectedXPosition - mCircleRadius + 10, mCompletedPaint);
                } else {
                    canvas.drawRect(mCenterX, preComplectedXPosition + mCircleRadius - 10, mCenterX,
                            afterComplectedXPosition - mCircleRadius + 10, mCompletedPaint);
                }
                //                canvas.drawLine(mCenterX, afterComplectedXPosition + mCircleRadius, mCenterX,
                //                        preComplectedXPosition - mCircleRadius, mCompletedPaint);
            } else {
                if (mIsReverseDraw) {
                    canvas.drawRect(mCenterX, afterComplectedXPosition + mCircleRadius - 10, mCenterX,
                            preComplectedXPosition - mCircleRadius + 10, mUnCompletedPaint);
                } else {
                    canvas.drawRect(mCenterX, preComplectedXPosition + mCircleRadius - 10, mCenterX,
                            afterComplectedXPosition - mCircleRadius + 10, mUnCompletedPaint);
                }
                //                canvas.drawLine(mCenterX, afterComplectedXPosition + mCircleRadius, mCenterX,
                //                        preComplectedXPosition - mCircleRadius, mUnCompletedPaint);
            }
        }
        //-----------------------画线-------draw line-----------------------------------------------
